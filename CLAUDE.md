...
- The viewer's `.image-with-prizes` wrapper lays `.image-frame` and the resolved-prizes list (`#prizes-row`, `.prizes-column`) side by side; `renderPrizesRow()` renders bare icon+amount rows (no group label in the UI) in whatever order the server already sorted them in — it does no sorting of its own.
